...
- Figma plugin: the Theme Editor preview's login card now shows the uicraft logo glyph on the brand-colored tile (instead of an empty brand square); the glyph color follows the derived on-brand token, so it stays contrast-correct as the theme changes.

### Added
- JS bundle: `UCChip` — filter chips inside `[data-chip-group]` toggle on click (`uc-chip-active` + `data-selected` + `aria-pressed`), and `UCTabs` — `[data-tab-group]` tab switching with roving-tabindex keyboard nav (arrows/Home/End). Both behaviors previously lived as docs-page-local scripts, so bundle consumers got static markup.
- Utilities: filled the gaps the docs already relied on — `uc-bg-success-primary` (+ `/10–/50` tints), `uc-border-success-primary/30`, `uc-border-warning-primary/30`, `uc-border-error-primary/30`, `uc-border-input`, `uc-bg-accents-yellow`, `uc-bg-border-strong`, `uc-bg-mains-quaternary/20`, `uc-text-generic-white/70,80`, `uc-text-success-primary/40`, `uc-text-mains-quaternary/20`, `uc-bg-gradient-to-t`. The success-colored chip dot was invisible everywhere because `uc-bg-success-primary` didn't exist.
...
- Tree View styles (`uc-tree-*`) moved from a docs-page-local `<style>` block into the design-system bundle — bundle consumers used to get unstyled markup.
- Site: Storybook link in the topbar navigation (`/storybook/`; hidden below 960px where the nav runs out of room).

### Fixed
- Border docs (both `/docs/border` and `/components/border`): the radius scale used a stale ad-hoc naming (`micro/tiny/small/medium/large/big/huge`) with values (2px, 6px) that no longer exist in the token set. Both pages now show the canonical scale used by the tokens and the Figma plugin — `none 0 / xs 4 / sm 8 / lg 12 (base) / xl 16 / 2xl 24 / 3xl 32 / 4xl 40 / full` — and preview swatches use the real `uc-rounded-*` utilities instead of hard-coded px classes. Added the missing `uc-rounded-xs` and `uc-rounded-4xl` utilities (`--radius-xs`/`--radius-4xl` already existed as tokens).
- Switch: a `uc-bg-*` color utility on an active switch was overridden by `.uc-switch.uc-active`'s brand background (specificity) — colored switches rendered blue until re-toggled. The brand fill now yields to color utilities.
- Avatar docs: the "lg" example used the `uc-avatar-xl` class, making lg and xl identical (80px); lg is 56px again.
- Progress/File Upload docs: several migrated inline-style classes referenced the removed `--accents-blue` token — fills and icons rendered transparent. Rewritten with semantic utilities (`uc-bg-success-primary`, `uc-bg-accents-yellow`) and the token reference fixed to `--info-primary`.
...
- Aspect Ratio: the docs page and search-index entry are gone (the `uc-aspect-*` utilities stay — the Media component uses them).
- Sheet component — dropped from the design system entirely: recipe, docs page, sidebar/nav links, showcase demo, `UCSheet` + `openSheet`/`closeSheet` globals in the JS bundle, `uc-sheet-*` CSS, the Figma-plugin registry entry, and the MCP spec/snippet.
...
- CSS cascade: `uc-*` utilities now come last in the bundle, so a utility on a component instance wins over the component's base style. Before, `.uc-toolbar-btn`'s fixed 36px width beat `uc-w-auto` (toolbar labels overlapped) and `.uc-avatar`'s grey background beat `uc-bg-info-primary` (initials avatars lost their color) — both visible on the docs site itself, surfaced via Storybook.
- Table: container radius reduced from `radius-2xl` (24px) to `radius-xl` (16px in the default theme; editorial 14px, rounded-sans 18px). Header rows use `surfaces-subtle` instead of `surfaces-moderate` (both `.uc-table thead` and the Data Table examples).
...
- Calendar day states `uc-outside`, `uc-range-start/middle/end` — the docs used these classes but they were never styled.
- Gradient utilities `uc-from-info-primary`, `uc-to-info-primary/70` — the transparent-header docs example referenced them; without a backdrop its white text sat on the page background in light mode.
...
- Auth template examples use the uicraft logo instead of a colored placeholder square.
- Sidebar Nav compact example: icons downsized to the standard `uc-w-4` (the `uc-w-4.5` class it used doesn't exist, so icons fell back to the SVG's 24px attribute).
...
### Fixed
- Dropdown menu (JS bundle): the outside-click closer was shutting combobox panels (they share the `uc-dropdown-menu` class) — a combobox dropdown flashed open and immediately closed; combobox-owned menus are now skipped.
...
- Site: 80 `uc-inline-*` demo rules dropped by an earlier `migrate:styles` rerun are back — the spacing/typography scale visuals, progress bars, resizable/scroll-view/empty-state/stats-card/select/rating/date-picker/collapsible/file-upload demos (and their `/docs/` mirrors) had been rendering unstyled. `scripts/migrate-component-styles.mjs` now preserves generated rules and extracted `<style>` sections that pages still reference instead of rebuilding both files from scratch, so already-migrated pages can't lose their styles again.
...
- Site: the "UC Utilities" / "Dark Mode" hero badges on `/components/` rendered black text on the brand fill — they still carried the pre-refactor ad-hoc outline classes on a base `uc-badge` that is now brand-filled. Migrated to `uc-badge-bordered` (with neutral border/text overrides), and the Badge overview card now previews the real `uc-badge` / `uc-badge-secondary` / `uc-badge-bordered` variants instead of hand-rolled color overrides.
